...
weights = {
    ...
    'Funding Valuation Score': st.sidebar.slider("Funding Valuation Score Weight", 0.0, 1.0, 0.15, 0.01),
    'Raised Score': st.sidebar.slider("Raised Score Weight", 0.0, 1.0, 0.20, 0.01),
    'Recent Financing Score': st.sidebar.slider("Recent Financing Score Weight", 0.0, 1.0, 0.20, 0.01),
    'HQ Location Score': st.sidebar.slider("HQ Location Score Weight", 0.0, 1.0, 0.05, 0.01),
    'Company Growth Score': st.sidebar.slider("Company Growth Score Weight", 0.0, 1.0, 0.10, 0.01),
  ...
  'Gender score': st.sidebar.slider("Gender Score Weight", 0.0, 1.0, 0.05, 0.01)
...
def score_vc(company):
    ...
    return 10 if len(matches) >=2  else 5 if len(matches) == 1 else 0
...
def check_hq_location(company):
    ...
    if region != 'europe':
        return 0
    elif country in ['france', 'germany']:
        return 5
    else:
        return 10
...
def evaluate_company_growth(row):
    ...
    if years_in_operation >= 5:
        ...
        # Companies younger than 5 years
        if growth > 400:
            return 10
        elif growth > 200:
            return 6
